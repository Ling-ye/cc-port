...
| `plugin` | A portable plugin snapshot or an explicit installation reference | Respect its source/reference track, scope, project identity, ownership, dependency and selector decisions. Do not upload plugin caches or silently convert a reference into copied content. |
| `instruction` | A tool-native user instruction such as configured Codex `AGENTS.md` or Claude `CLAUDE.md` | Use profile-aware asset workflow only. Preserve native meaning; do not translate between tools or expose project-level instructions as user-global upload candidates. |
| `memory` | An exact Markdown memory-directory snapshot | Use profile-aware asset workflow only. Keep ownership markers outside the content tree, scan every Markdown file for suspected secrets, and require the configured local install-name mapping for Claude project-slot layouts. |
