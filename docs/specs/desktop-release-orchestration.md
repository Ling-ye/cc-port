...
- 构建内部继续使用仓库 `.venv`，并分别生成 PyInstaller Desktop API sidecar
  `cc-port-desktop-api.exe` 与公开 CLI/MCP agent `cc-port.exe`；桌面客户端仍由
  Tauri/Cargo 构建。
- 环境准备与完整发布分别只有一个公开入口。
...
# 强制依赖同步、sidecar clean build 与 Cargo 主程序重新链接；agent 始终 clean build
Set-ExecutionPolicy -Scope Process Bypass -Force; & .\scripts\release-desktop.ps1 -Clean
```

- `setup.ps1` 默认打印环境和待执行动作后立即继续，不读取 `y/n`，也没有二次确认；
  `-NonInteractive` 只给 WinGet 安装追加 `--silent --disable-interactivity`，不改变
  CC Port 自身流程。`release-desktop.ps1` 的两条 setup 调用都必须显式传入该开关，
  使发布期间的 WinGet 安装保持静默。发布入口自身继续接受 `-NonInteractive` 以兼容
  旧调用，但该参数不再改变行为。
- `-CheckOnly` 不安装系统包、不创建或修改 `.venv`、不运行 pip/npm 安装，并以退出码表示环境是否完整。
- `release-desktop.ps1` 必须先调用 `setup.ps1`，不能复制第二套环境安装逻辑。
- 默认入口必须复用验证通过的依赖、Desktop API sidecar 与 Cargo 产物；公开
  CLI/MCP agent 当前不使用 sidecar cache，每次发布都执行 PyInstaller clean build。
  `-ForceSync`/`-Clean` 是显式绕过路径，均不得执行 `cargo clean`。
...
3. 生成缺失图标，并验证 Desktop API sidecar 的输入指纹、缓存产物哈希和隔离
   `operation_history_page` JSON `ok` 冒烟；未命中或缓存冒烟失败时 clean 重建、
   重新冒烟并刷新缓存。sidecar 必须收集 canonical AI Skill 数据，保证桌面计划能够
   安装随包工作流。随后始终 clean 构建公开 CLI/MCP agent，并验证
   `cc-port.exe --help`、`cc-port mcp --stdio` 初始化、工具发现以及
   `cc_port_status` 调用。
4. 默认删除 Tauri bundle、目标 Desktop API sidecar 和目标 public agent，保留
   Cargo 管理的桌面主程序；`-Clean` 额外删除顶层主程序以强制重新链接，但不清理
   Cargo 依赖编译产物。
5. 把验证通过的源 sidecar 和源 agent 分别复制到 Tauri
   `target/release/cc-port-desktop-api.exe` 与 `target/release/cc-port.exe`，并在
   Tauri 构建前分别校验源、目标 SHA-256 一致。
...
8. 构建后再次验证 Tauri 目标 Desktop API sidecar 与源 sidecar 的 SHA-256
   一致，并以 UTF-8、UTF-16LE 和 UTF-16BE 检查桌面 EXE、sidecar 与 public
   agent 都不包含重映射前的用户目录、Cargo/Rustup 目录或仓库路径。随后把三种
   EXE 以及 Tauri bundle 中的 MSI、NSIS 和辅助文件复制到同级临时发布目录。
9. 对临时目录中的 sidecar 再次运行隔离状态 JSON `ok` 冒烟，对临时 agent 再次
   运行 CLI/MCP 冒烟。哈希清单包含桌面 EXE、sidecar、public agent 以及发现到的
   全部 MSI 和 NSIS；标准单 MSI、单 NSIS 构建因此计算五个内部发布文件、五类产物
   的 SHA-256。bundle 辅助文件可以随 staging 进入正式本地目录，但不进入已验证
   产物哈希清单。
10. 全部验证成功后事务式切换正式目录。
...
- MSI、桌面 EXE、Desktop API sidecar EXE 和 public CLI/MCP agent EXE 只作为
  本地验证产物，不进入手工上传目录。
- 手工上传目录与正式目录分别事务式替换；公开目录生成失败不得损坏已验证的正式目录或上一次公开目录。
- 测试、构建、安装包验证、sidecar 冒烟或 agent CLI/MCP 冒烟失败不得覆盖上一次
  已验证目录。
...
- 依赖与 Desktop API sidecar 缓存记录必须使用版本化 JSON envelope 和同目录原子
  写入；记录缺失、损坏、不完整、指纹或产物哈希不符时均按 miss 处理。public
  agent 当前每次 clean build，不读取或写入该 sidecar cache。
...
- 每次发布尝试必须在 `build/metrics/` 保存成功/失败、总耗时、逐阶段退出码/缓存状态/
  日志和成功产物哈希。标准成功发布记录五个内部验证文件与公开目录两个文件，共七条
  artifact 记录；若 Tauri 产生多个 MSI 或 NSIS，则内部记录按实际发现且去重后的
  安装包数量增加。
...
- 默认 `setup.ps1` 在展示 Required actions 后直接执行，不得出现 `Read-Host`、
  `PromptForChoice` 或其他 `y/n` 确认；`-NonInteractive` 只控制 WinGet 的
  `--silent --disable-interactivity` 参数。
...
- 一条 `release-desktop.ps1` 命令完成所有门禁，生成并哈希桌面 EXE、Desktop API
  sidecar、public CLI/MCP agent、MSI 和 NSIS；sidecar JSON 冒烟与 agent 的 CLI
  help、MCP 工具发现及 `cc_port_status` 冒烟都必须通过。
...
- Tauri `target/release` 中的桌面 EXE、Desktop API sidecar 与 public agent 均不得
  包含构建用户名、用户目录、Cargo/Rustup 目录、仓库绝对路径或 CI 工作目录；只检查
  NSIS 压缩层不能代替这三种原始二进制的检查。
...
- 三次候选质量门禁中位数为 27.377 秒，相对 51.285 秒基线减少
  ...
  26.50%。
- 三次候选的依赖和 sidecar 缓存均命中，全部质量门禁、冒烟、哈希与当时四类
  内部产物验证通过，因此保留 xdist 方案。该历史实验早于 public CLI/MCP agent
  加入发布编排；当前标准发布验证五类内部产物，不能把当时的四类计数当作现行合同。
