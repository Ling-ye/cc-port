...
桌面端会用 Toast 提示异步任务结果，并在顶栏任务中心保留本次应用会话最近的任务记录。任务历史不会跨应用重启保存；运行中的任务始终保留，已完成记录最多保留 50 条。环境诊断（包括启动后台检测和设置页手动重检）不产生 Toast，也不进入任务中心；结果通过设置页状态和诊断弹窗反馈。
...
- 远端刷新与本地扫描可在后台并行，各自只禁用自己的来源按钮；手动、自动或任务重试触发的同来源操作会加入当前任务，不重复请求或发布。任一来源完成都会立即重算并发布一次双端 diff，旧响应不会覆盖较新的安装状态；刷新期间的配置或资源变更会在任务结束后合并补刷，补刷期间的新变更继续安排下一轮。
